...
| `UT-005` | `MOD-002` | request USPP, PAW, spin, nonzero charge, unsupported XC, or mismatched UPF metadata | `ArgumentError` names the rejected feature |
| `UT-006` | `API-003`, `BAS-001` | enumerate plane waves for a small analytic cubic-cell case | every and only `G` satisfying the declared cutoff convention is present |
...
| `UT-011` | `API-006`, `RSP-002` | use invalid atom/direction or q that does not permute the k mesh | `ArgumentError` names the invalid component |
...
| `UT-013` | `QEI-002` | parse equivalent `bohr`, `angstrom`, `alat`, Ry, unified-mass, and native atomic-unit quantities | canonical lattice, positions, masses, and cutoff agree within conversion roundoff |
...
| `UT-015` | `QEI-005` | parse unsupported calculation, `ibrav`, occupation, spin, card, and non-default keyword cases | every case raises `ArgumentError` naming the field or card |
...
| `PT-006` | `API-011`, `OBS-002` | displace a Si atom by symmetric decreasing steps and compare energy differences | finite-difference forces converge to `forces(gs)` before numerical noise dominates |
...
| `RT-002` | `API-016`, `RSP-003` | force response nonconvergence with an impossible tolerance/resource bound | `ErrorException` is raised rather than returning a false converged result |
...
| `RT-011` | `POL-003` | construct direction-dependent non-analytic Gamma limits from reported polar tensors | LO and TO limits match the pinned QE observations within `2 cm^-1` |
...
| `IT-005` | all API IDs and clauses through the mapping below | repeat the complete workflow on held-out crystal parameters, UPF hashes, cutoffs, meshes, and q vectors | private suite passes without relying on visible golden values |
